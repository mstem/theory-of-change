...
- Status: sections 1 and 2 signed off 10 September 2026; steps 1 to 5 done, step 6 waits on a deploy
...
3. **Tune the cap and the rubric together** against the three test cases in the success criteria. `max_uses` and the rubric wording interact — a tight cap makes the model ration searches in ways that change which claims get grounded.
   ...
   Measured 11 September 2026. Four theories, one per criterion, run at `max_uses: 3` and again at 5. **The cap stays at 3.**

   The model spends whatever ceiling it is given: 3 of 3 in all four runs, 5 of 5 in all four runs. It said out loud that it had run out in three of the four runs at 3, and at 5 it still finished by saying it wanted more. So the ceiling does not decide when the model feels it has enough; it decides what the analysis costs and how long it takes. Going from 3 to 5 raised the average from $0.18 to $0.27 (47 percent) and the worst run from 246 seconds to 966, because the extra searches pushed into upstream rate limiting, where the model spends minutes waiting and retrying rather than searching. One run at 5 held its connection open long enough for the client to abort it.

   Quality did not move with the cap. Search results roughly doubled (22 to 30 per run at 3, 40 to 49 at 5) without changing any criterion outcome, and the nonviolent-mobilisation case came back slightly worse at 5: 2 findings a side rather than 3, and the landmark cited through a magazine's coverage rather than the book. This answers open question 3 against the guess recorded there: source-first ordering does not argue for a higher cap.

   Criterion results at 3, all four runs with zero search errors: the landmark case returns Chenoweth and Stephan (2011) in support and the 2020 NAVCO finding that success rates fell below 34 percent after 2010 against, which satisfies criteria 2 and 3 in a single analysis. The school-phones case puts the 2015 LSE and 2024 Norway studies in support and the 2025 Birmingham study in *The Lancet* against, criterion 3 again. The bed-nets case returns 3 findings for and 2 against at strength 88, both against being real caveats about insecticide resistance and the gap between owning a net and using one, which satisfies criterion 5. The AI-elections case satisfies criterion 1: the 2023-24 Pew claim that motivated this document is gone, replaced by 2024 and 2026 sources.

   One thing the cap does not explain. That AI-elections run returned an **empty** `evidence_for` column, and at 3 the model had said it wanted one more search for the supporting side. At 5 it ran those searches, found a *Science Advances* meta-analysis on voter information, and still returned nothing in that column, while an earlier run of the same theory at the same cap had found a randomised chatbot trial and a PNAS study for it. So the empty column is run-to-run variance in the model's judgment about whether evidence for voter information in general supports a claim about chatbots in particular, not a search budget that ran out. It is the honest reading of the rubric, and it is also the case where an empty column is most likely to mislead, since the column is a claim. Worth watching in step 6 rather than fixing by widening the cap.
...
3. ~~Is `max_uses: 3` right?~~ **Answered 11 September 2026: yes, and the reasoning recorded here was wrong.** Measured at 3 and at 5 across four theories. The model exhausts any ceiling it is given, so the cap buys cost and latency rather than knowledge: 47 percent more money, up to four times the wall-clock, no criterion outcome changed. See step 3 above.
