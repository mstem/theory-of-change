...
**Rate limiting.** `/api/analyze` is capped at 60 requests per 15 minutes per IP. At $0.06 per uncached analysis that is $3.60 per IP per window, up from near zero. Recalculate and decide before ship, not after.
